const ParentSettingsSection = ({ userData }) => {
    ...
    // Personal info state
    const [personalInfo, setPersonalInfo] = React.useState({ firstName: '', lastName: '', dateOfBirth: '', street: '', city: '', state: '', postalCode: '', country: '' });
    ...
    React.useEffect(() => {
        fetch('/dashboard/parent/settings/personal-info')
        ...
        fetch('/dashboard/parent/settings/2fa/status')
            .then(r => r.ok ? r.json() : null)
    ...
    const handleToggle2FA = async () => {
        ...
        setTwoFAMsg('');
        try {
            const res = await fetch('/dashboard/parent/settings/2fa/toggle', { method: 'POST' });
    ...
    const handleSavePersonalInfo = async (e) => {
        ...
        setPiMsg('');
        try {
            const res = await fetch('/dashboard/parent/settings/personal-info', {
                method: 'POST',
